...
function startPeriodicTracking() {
    ...
    // Set up new interval to periodically save to localStorage only
    trackingInterval = setInterval(function() {
        if (sectionEvents.length > 0 && !isTrackingPaused) {
            // Only save to localStorage, don't send data to server yet
            saveEventsToLocalStorage();
        }
    }, TRACKING_INTERVAL);
}
...
function sendTrackingData(isSync = false) {
    ...
    // If it's not a page unload event, just save to localStorage and return
    if (!isSync) {
        saveEventsToLocalStorage();
        return;
    }
    
    // For page unload events, proceed with sending the data
    const username = getUsername();
