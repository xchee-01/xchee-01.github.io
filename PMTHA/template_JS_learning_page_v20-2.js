...
const TRACKING_INTERVAL = 30000; // 15 seconds for periodic tracking (changed from 5000)
const HEARTBEAT_INTERVAL = 1000; // 1 seconds for heartbeat (changed from 5000)
...
function initializeTracking() {
    ...
    // Save session state on unload - UPDATED: added both unload and beforeunload
...
// Start periodic tracking - MODIFIED to add periodic sending
function startPeriodicTracking() {
    ...
    // Set up new interval to periodically save to localStorage and send data if threshold reached
    trackingInterval = setInterval(function() {
        if (sectionEvents.length > 0 && !isTrackingPaused) {
            // Always save to localStorage
            saveEventsToLocalStorage();
            
            // If we have enough events, try to send them to the server
            if (sectionEvents.length >= EVENT_BATCH_THRESHOLD) {
                console.log(`Periodic tracking threshold reached: ${sectionEvents.length} events`);
...
function handleUserReturn() {
    ...
    // Save to localStorage (changed from sendTrackingData())
    saveEventsToLocalStorage();
...
function setupVisibilityTracking() {
    ...
    document.addEventListener(visibilityChange, function() {
        ...
        if (isPageVisible) {
            ...
            // Save any pending heartbeat events to localStorage before page is hidden
...
// Send tracking data to server with batching - MODIFIED for better reliability
function sendTrackingData(isSync = false) {
    // Log debug information
    console.log('Send tracking data called with isSync =', isSync);
    console.log('Events count:', sectionEvents.length, 'isPaused:', isTrackingPaused);
    
    // Don't proceed if there's no data
    if (sectionEvents.length === 0) {
        console.log('No events to send, returning early');
        return;
    }
    
    // If isSync is true (page unload) but tracking is paused, temporarily unpause it to allow sending
    const wasPaused = isTrackingPaused;
    if (isSync && isTrackingPaused) {
        console.log('Temporarily unpausing tracking for page unload data send');
        isTrackingPaused = false;
    }
    
    // Now check if tracking is still paused (will only be true if not isSync and was already paused)
    if (isTrackingPaused) {
        console.log('Tracking is paused, skipping data send');
        return;
    }
    ...
    // If it's not a page unload event and we don't have enough events, just save to localStorage and return
    if (!isSync && sectionEvents.length < EVENT_BATCH_THRESHOLD) {
        saveEventsToLocalStorage();
        return;
    }
    
    // For page unload events or if we have enough events, proceed with sending the data
    const username = getUsername();
    ...
    console.log(`Sending ${processedEvents.length} processed events to server`);
    
    // Use beacon API for unload events
    if (navigator.sendBeacon && isSync) {
        try {
            const blob = new Blob([JSON.stringify(data)], { type: 'application/json' });
            const beaconSuccess = navigator.sendBeacon(APPS_SCRIPT_URL, blob);
            
            // Check if beacon was successfully queued
            if (beaconSuccess) {
                console.log('Beacon successfully queued');
                // Clear localStorage after successful send
                localStorage.removeItem(LOCAL_STORAGE_KEY);
            } else {
                console.warn('Beacon send returned false, data kept in localStorage');
                // Put events back in the queue and save to localStorage
        ...
        // Restore previous pause state if we temporarily unpaused
        if (isSync && wasPaused) {
    ...
    // Use regular fetch for non-unload events or fallback if beacon not available
    if (!isSync || !navigator.sendBeacon) {
        // For non-unload events, use fetch
        if (!isSync) {
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    console.warn('Server responded with error, data kept in localStorage');
                    // Put events back in the queue
                    sectionEvents = [...eventsCopy, ...sectionEvents];
                    saveEventsToLocalStorage();
                }
        ...
        // Fallback to synchronous XHR for older browsers during unload
        try {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', APPS_SCRIPT_URL, false); // false for synchronous
            xhr.setRequestHeader('Content-Type', 'application/json');
            xhr.send(JSON.stringify(data));
            
            if (xhr.status >= 200 && xhr.status < 300) {
                console.log('Data sent successfully via synchronous XHR');
                // Clear localStorage after successful send
                localStorage.removeItem(LOCAL_STORAGE_KEY);
            } else {
                console.warn('XHR error response:', xhr.status);
                // Put events back in the queue
                sectionEvents = [...eventsCopy, ...sectionEvents];
                saveEventsToLocalStorage();
            }
        } catch (error) {
            console.error('Synchronous XHR error:', error);
            // Put events back in the queue
            sectionEvents = [...eventsCopy, ...sectionEvents];
            // Update localStorage
            saveEventsToLocalStorage();
...
// Handle page unload - MODIFIED to ensure data gets sent
function handlePageUnload() {
    console.log('Page unload event triggered');
    console.log('Current tracking status - isPaused:', isTrackingPaused);
    
    // Ensure tracking is not paused during page unload, to guarantee data sending
    isTrackingPaused = false;
    ...
    // Always send data on page unload, regardless of batch size or tracking state
    if (sectionEvents.length > 0) {
        // Force isSync to true to ensure synchronous sending
